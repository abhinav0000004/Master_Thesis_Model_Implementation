...
def get_pose2D(video_path, output_dir, video_name):
    ...
    output_npz = output_dir + video_name + '_2D.npz'
...
def get_pose3D(video_path, output_dir, video_name):
    ...
    keypoints = np.load(output_dir + video_name +'_2D.npz', allow_pickle=True)['reconstruction']

    cap = cv2.VideoCapture(video_path)
    video_length = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
    output_3d_all = []  # Initialize list for 3D keypoints
    ...
    for i in tqdm(range(video_length)):
        ...
        output_3d_all.append(post_out)  # Append 3D keypoints
    ...
    # Save 3D keypoints as npz
    output_3d_all = np.stack(output_3d_all, axis=0)
    output_npz = os.path.join(output_dir, video_name +'_3D.npz')
    np.savez_compressed(output_npz, reconstruction=output_3d_all)

    print('3D keypoints saved successfully!')
...
def main():
    ...
    for vid in video_files:
        ...
        trim_video_with_ffmpeg(video_path, max_duration=10)

        # Output directory for pose estimation tasks
